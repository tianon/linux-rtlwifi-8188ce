			...
			struct ieee80211_sta *sta)
{
	...
	if (b_is_cck) {
		...
		/* (3)EVM of HT rate */
		if (pstats->b_is_ht && pstats->rate >= DESC92C_RATEMCS8 &&
	...
	if (b_is_cck)
		pstats->signalstrength = (u8)(_rtl92ce_signal_scale_mapping(hw,
			pwdb_all));
	else if (rf_rx_num != 0)
		pstats->signalstrength = (u8)(_rtl92ce_signal_scale_mapping(hw,
...
static void _rtl92ce_process_pwdb(struct ieee80211_hw *hw,
		struct rtl_stats *pstats, struct ieee80211_sta *sta)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
		...
		struct ieee80211_sta *sta)
{
	...
	_rtl92ce_process_pwdb(hw, pcurrent_stats, sta);
		...
		u8 *pdesc, struct rx_fwinfo_92c *p_drvinfo)
{
	...
	_rtl92ce_query_rxphystatus(hw, pstats, pdesc, p_drvinfo,
				   ...
				   b_packet_beacon, sta);
	_rtl92ce_process_phyinfo(hw, tmp_buf, pstats, sta);
	rcu_read_unlock();
}
